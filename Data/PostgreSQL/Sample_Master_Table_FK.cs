...
namespace N_Ter.PostgreSQL.Customizable
{
    public class Sample_Master_Table_FK : Base.Sample_Master_Table_FK
    {
        ...
        public override DS_Master_Tables ReadAsMaster()
        {
            ...
            NpgsqlDataAdapter adp = new NpgsqlDataAdapter("SELECT Master_ID AS Data_ID, Master_Data AS Data FROM tblzm_sample_with_foreign_key", con);
        ...
        {
            NpgsqlConnection con = new NpgsqlConnection(_strConnectionString);
            NpgsqlDataAdapter adp = new NpgsqlDataAdapter("SELECT Master_ID AS Data_ID, Master_Data AS Data FROM tblzm_sample_with_foreign_key WHERE Foreign_Key_ID = " + Foreign_Key_ID, con);
            DS_Master_Tables ds = new DS_Master_Tables();
            adp.Fill(ds.tblData);
            objComDB.CloseAdapter(ref con, ref adp);
            return ds;
        ...
        {
            NpgsqlConnection con = new NpgsqlConnection(_strConnectionString);
            NpgsqlDataAdapter adp = new NpgsqlDataAdapter("SELECT Master_ID AS Data_ID, Master_Data AS Data FROM tblzm_sample_with_foreign_key WHERE Foreign_Key_ID = " + Foreign_Key_ID + " AND Foreign_Key_2_ID = " + Foreign_Key_2_ID, con);
            DS_Master_Tables ds = new DS_Master_Tables();
            adp.Fill(ds.tblData);
            objComDB.CloseAdapter(ref con, ref adp);
            return ds;
